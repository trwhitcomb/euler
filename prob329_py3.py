...
def compute_probabilities(n, state):
    ...
    #
    # TODO: Perhaps I should cast this as just the probability of getting
    #       a P, and then if I need to compute P(N) I can just do 1-P(P).
...
def solve(outcome='PPPPNNPPPNPPNPN'):
    ...
    for croak in outcome:
        ...
        # Need to add an update here so we compute a Bayesian posterior
        # for the state given the croak we just observed.  The vector of
        # state probabilities needs to be updated before advancing to the
        # next time step.  i.e. compute probabilities based on the prior,
        # then update to a posterior, then advance the posterior.
        x = np.dot(A, x)
    print(float(p))
...
def run(ref_string, num_trials=50000):
    results = [trial(ref_string) for _ in range(num_trials)]
...
def run_parallel(ref_string, num_trials=50000):
    # Run like it would have under Python 2, whose default start method was
    # 'fork'.  macOS's Python 3 defaults to 'spawn', under which result_queue
    # cannot be handed to the Pool via initargs (it must be inherited, which
    # is exactly what fork does).  Using a fork context restores that.
    ctx = multiprocessing.get_context('fork')
    result_queue = ctx.Queue()

    pool = ctx.Pool(initializer=parallel_init,
                    initargs=(result_queue,))
    ref_strings = (ref_string for _ in range(num_trials))
...
if __name__ == "__main__":
    ...
    probs = [run_parallel(outcome, num_trials=500000) for _ in range(2)]
